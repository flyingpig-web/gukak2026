/* 자동 생성: img/ 폴더 스캔 결과. 이미지 추가/삭제 시
   `python3 scripts/gen-preload-manifest.py` 로 재생성.
   화면 흐름 순서의 전체 이미지 매니페스트(전역 프리페치용). */
window.AR_MANIFEST = {
  "common": [
    "img/00_common/btn_back.png",
    "img/00_common/btn_exit.png",
    "img/00_common/btn_home.png",
    "img/00_common/btn_info.png",
    "img/00_common/btn_play.png",
    "img/00_common/btn_retry.png"
  ],
  "flow": [
    {
      "page": "index.html",
      "images": [
        "img/01_title/bg_title.png",
        "img/01_title/btn_start.png"
      ]
    },
    {
      "page": "main.html",
      "images": [
        "img/02_main/bg_main.png",
        "img/02_main/btn_exp1.png",
        "img/02_main/btn_exp1_on.png",
        "img/02_main/btn_exp2.png",
        "img/02_main/btn_exp2_on.png",
        "img/02_main/text_exp1.png",
        "img/02_main/text_exp2.png",
        "img/02_main/text_main.png"
      ]
    },
    {
      "page": "exp1.html",
      "images": [
        "img/03_exp1/arrow.png",
        "img/03_exp1/bg_exp1.png",
        "img/03_exp1/daegeum.png",
        "img/03_exp1/daegeum2.png",
        "img/03_exp1/drill.png",
        "img/03_exp1/end_bg.png",
        "img/03_exp1/end_image.png",
        "img/03_exp1/end_text.png",
        "img/03_exp1/exp1_tutorial.png",
        "img/03_exp1/hole1.png",
        "img/03_exp1/hole2.png",
        "img/03_exp1/hole3.png",
        "img/03_exp1/hole_effect.png",
        "img/03_exp1/label-cheonggong.png",
        "img/03_exp1/label-chilseonggong.png",
        "img/03_exp1/label-chwigu.png",
        "img/03_exp1/label-galdaecheong.png",
        "img/03_exp1/label-jigong.png",
        "img/03_exp1/line_hole1.png",
        "img/03_exp1/line_hole2.png",
        "img/03_exp1/line_hole3.png",
        "img/03_exp1/reed_membrane.png",
        "img/03_exp1/title_exp1.png"
      ]
    },
    {
      "page": "exp2.html",
      "images": [
        "img/04_exp2/bg_level1.png",
        "img/04_exp2/bg_level2.png",
        "img/04_exp2/bg_level3.png",
        "img/04_exp2/boat_level1.png",
        "img/04_exp2/boat_level2.png",
        "img/04_exp2/boat_level3.png",
        "img/04_exp2/btn_blowing.png",
        "img/04_exp2/end_main.png",
        "img/04_exp2/end_text.png",
        "img/04_exp2/exp2_title.png",
        "img/04_exp2/exp2_tutorial.png",
        "img/04_exp2/gage_off.png",
        "img/04_exp2/gage_on.png",
        "img/04_exp2/rendition1.png",
        "img/04_exp2/rendition2.png",
        "img/04_exp2/rendition3.png",
        "img/04_exp2/rendition4.png",
        "img/04_exp2/rendition5.png",
        "img/04_exp2/rendition6.png",
        "img/04_exp2/rendition7.png"
      ]
    },
    {
      "page": "info.html",
      "images": [
        "img/05_info/bg_info.png",
        "img/05_info/btn_cheonggong.png",
        "img/05_info/btn_chilseonggong.png",
        "img/05_info/btn_chwigu.png",
        "img/05_info/btn_close.png",
        "img/05_info/btn_jigong.png",
        "img/05_info/btn_next.png",
        "img/05_info/daegeum.png",
        "img/05_info/popup_cheong-galigae.png",
        "img/05_info/popup_cheonggong.png",
        "img/05_info/popup_chilseonggong.png",
        "img/05_info/popup_chwigu.png",
        "img/05_info/popup_galdaecheong.png",
        "img/05_info/popup_jigong.png",
        "img/05_info/speech_bubble.png",
        "img/05_info/title_info.png"
      ]
    }
  ]
};
